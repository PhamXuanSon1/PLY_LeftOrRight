...
public class SpineBoneIKControl : MonoBehaviour
{
	...
	public float scalingX = 1f;

	public float scalingY = 1f;
	...
	{
		if (_bone != null && !(target == null))
		{
			float localX;
			float localY;
			if (_bone.Parent != null)
			{
				_bone.Parent.WorldToLocal(TargetPosition.x + positionOffset.x, TargetPosition.y + positionOffset.y, out localX, out localY);
			}
			else
			{
				skeletonAnimation.Skeleton.RootBone.WorldToLocal(TargetPosition.x + positionOffset.x, TargetPosition.y + positionOffset.y, out localX, out localY);
			}
	...
	public void DisableIKControl(bool instant = false)
	{
		...
		_weightTween = DOTween.To(() => _ikWeight, delegate(float x)
		{
			_ikWeight = x;
		}, 0f, transitionDuration).OnComplete(delegate
		{
			skeletonAnimation.UpdateLocal -= UpdateIK;
			_bone?.SetToSetupPose();
		});
